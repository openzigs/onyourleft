...
"""
The realistic rider's BODY: MakeHuman's CC0 base mesh, rigged with its own
default skeleton and weights, cut down for a phone -- #369, #430.
...
bone at the joint positions `src/game/rider-pose.ts` derives from
`src/game/bicycle.ts` -- the hips on the saddle, the hands on the hoods, and
each knee and foot from `legBones(crankAngle)`. One source for where a leg is,
rather than a pose baked here that `bicycle.ts` could drift from; and the legs
therefore turn exactly when the HUD shows a cadence, which is #349's rule.

ADR 0026 D-5: the input is pinned by `inputs.lock.json` (MakeHuman at one
commit) and the tool by `sources.ts` section PINNED_BLENDER.
"""
...
# Which garment a bone's dominant vertices wear, in LINEAR light (Blender's
# colour attribute takes linear values and glTF's COLOR_0 is linear). The
# jersey and the bib shorts are `bicycle.ts` section RIDER_PALETTE's jersey
# (0x2f6fed) and tyre (0x22262b) converted from sRGB, so the realistic rider
# wears the stylised rider's colours and the bot's and the ghost's tints (#368)
# land on the same kit.
JERSEY = (0.0284, 0.159, 0.846, 1.0)
BIB = (0.016, 0.019, 0.024, 1.0)
SKIN = (0.52, 0.32, 0.23, 1.0)
SHOE = (0.8, 0.8, 0.8, 1.0)
GARMENT = {
