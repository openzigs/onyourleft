...
/**
 * The stretch of road the realism spike is measured on — #457.
 *
 ...
 * and `settlements.ts`, which #468 added — so the realism sits in exactly the
 * world the product draws: a valley the road descends into, a level floor
 ...
 */
export function realismElevation(along: number): number {
 ...
 */
export function realismHeading(along: number): number {
  return 0.45 * Math.sin((along / 700) * Math.PI * 2);
}

/** The route, as the product profiles it. */
export function realismRoute(): RouteProfile {
  ...
  for (let along = 0; along <= ROUTE_METRES; along += step) {
    points.push({
      ...
      elevation: altitudeMetres(realismElevation(along)),
    });
    const heading = realismHeading(along + step / 2);
