...
`pacer/` is the bot pacer's arithmetic (#92) — **a synthetic rider ahead of you on the route, riding
a chosen w/kg, that will not wait for you.** It is split across two packages by the dependency
direction and by nothing else: the rule needs the route profile, which is here, and the rider needs
the physics tick, which is in `@onyourleft/physics` and depends on this package. So `pacing.ts`
decides how hard the bot is pedalling and `@onyourleft/physics` §`advanceBot` decides where that gets
it, through **the same `advance` a rider's own ride runs through**.

⚠️ **That the bot is synthetic is a constraint, not a description of how it happens to be
implemented today.** [ADR 0007](../../docs/adr/0007-patent-posture.md) D4 is the reason: Peloton's
US 10,486,026 claim 1 needs "archived performance data … previously generated by the other users"
and US 11,170,886 claim 19 needs "live performance parameters" from a live session, and a bot has
neither because **there is no recorded ride behind it, from this rider or from anyone else.** Its
whole input is a target intensity, a fixed 75 kg, a `RouteProfile` — elevation as a function of
*distance*, with no time axis and no athlete in it — and an elapsed time the caller supplies.

#92 requires that to be *checked rather than assumed*, and it is, in three places:

- **At compile time.** `BotPacerPlan` is constrained by a mapped type that maps every non-scalar
  field to `never`, applied as a constraint rather than as a conditional — see the ⚠️ in `pacing.ts`
  about why the other direction enforces nothing. Adding `readonly samples: readonly number[]` to
  the plan makes `pacing.ts` itself fail with `TS2344`.
- **At run time.** `pacing.test.ts` walks the object a bot is actually handed and fails on any key
  outside a named allowlist, so a new field cannot arrive without somebody reading its name.
- **By the package boundary.** This package may name no platform API at all, so there is no I/O
  through which stored data could reach the bot other than those parameters.

There is no leaderboard, no ranking and no comparison against any other person: the only comparison
the feature makes is the rider against the bot, and the bot is arithmetic. A test asserts the
exported surface of `pacing.ts` and `gap.ts` names no athlete, activity, effort, ghost or ranking.

⚠️ **`gap.ts` compares two odometers and never wraps either.** That is the behaviour on a loop: a
bot a full lap ahead reads as a lap's worth of metres ahead rather than as level with you.
`route/profile.ts` §`distanceOnRoute` is what wraps, and it answers a different question — *what
gradient is here*, not *who is in front*.
