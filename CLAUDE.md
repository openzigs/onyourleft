...
The boundary _is_ the directory, so it is checkable by path rather than by reading manifests. This is
...
- ⚠️ **An "it lands under `apps/`" argument is checked with `pnpm why <pkg> --recursive`, and a
  ...
  under pnpm's isolated `node_modules` it returns _not resolvable_ for every transitive dependency,
...
would be a _worse_ misdeclaration than leaving them bare — it claims authorship we do not have, and
...
- A generator that writes a file under a **new** name lands _outside_ the list and fails
...
header is awkward here" is not a reason: a file this repository authors _or edits_ carries the
...
| `SPIKE003` | `docs/spikes/` is absent, or holds no write-up. ⚠️ This is what stops the two above passing over nothing: a rule that walks a path which is not there reports clean for ever, which is `LIC006`'s reason applied to a directory and #142's shape applied to a selector — failing closed against _deleting_ the directory and **open** against _renaming_ it. The directory is therefore **asserted**, as `check-wiring.mjs` asserts `WATCHED_PREFIXES` and as `ASSET005` asserts the manifest is present. Measured 2026-09-23: removing its two `report` calls turns exactly two of the suite's 206 cases red and nothing else; pointing the walk at a suffix nothing uses makes it fire on this repository                                                                                                                                                                     |
...
| `DEP002` | **third-party copyleft in the _distributed_ closure of an application.** [ADR 0025](docs/adr/0025-app-store-additional-permission.md) D-5: an app ships through an app store under an additional permission this project's copyright holders grant (`COPYRIGHT`), and that permission cannot cover anybody else's GPL, LGPL or AGPL code. ⚠️ **Until ADR 0025 this was permitted**, and a reviewer who remembers _"GPL is fine under `apps/`"_ is remembering the **build-time** half, which still is. One such dependency is one third party able to have the app removed from a store, which is what a single copyright holder did to VLC in 2011 |

The stack the workspace is built on is decided in ADR 0005 and is not open:

| Concern              | Decision                                                        |
| -------------------- | --------------------------------------------------------------- |
| Language             | TypeScript **6.0.3** — _not_ 7.x, see §8                        |
...
- **`react-router` and the rest of the runtime dependency list in ADR 0005.** See the dependency
  paragraph below the next heading for what _is_ installed.
...
- **`packages/sensors/src`** ([#39](https://github.com/openzigs/onyourleft/issues/39)) holds the
  ...
  **`packages/sensors/web-bluetooth`** ([#40](https://github.com/openzigs/onyourleft/issues/40))
  ...
  `writeValueWithResponse`; `gatt.ts` declares the unacknowledged sibling _and never calls it_ so
...
- **`packages/store`** holds athletes, activities, laps and privacy zones
  ...
  **round-trip persistence harness**
  ...
  which is the only record in the store that holds a whole _computed_ artefact rather than
...
- **`packages/physics`** ([#88](https://github.com/openzigs/onyourleft/issues/88)) holds the
  **Martin et al. 1998** power/speed model: the six force terms one exported function at a time
  ...
  rule with `advance`, so a bot and a rider go through the same tick. Nothing _renders_ a speed
...
pins it by version** under the heading _"What does NOT change"_, and an ADR is not reversed by a
...
because the import and export screen is the codec's first _production_ caller rather than a test's.
...
always()` design, observed rather than asserted. And a defect that trips an _earlier_ step proves
...
⚠️ **#24's own wording for this asks for something §5 forbids.** The criterion reads _"the
coverage gate is enforced and demonstrated to fail below threshold"_, and it **predates §5 and
...
the lint list cannot be: with no ES library entry and no `@types` package in scope, _any_ name from
...
⚠️ **`packages/sensors` narrows through a _second_ tsconfig, not its main one**, for the reason
...
> pattern containing no slash matches _any_ path segment. The derived Node-builtin list therefore
...
is that _automated accessibility checks run on every route in CI and fail the build on a violation_.
...
[#142](https://github.com/openzigs/onyourleft/issues/142) the filter was the _directory_ name, which
...
- _A test inside an `a11y`/`accessibility` directory that lacks the convention_ is **rule 3**, and
  it is what catches #142's own regression. Renaming the directory leaves the old filter matching
  the _filename_, so the conventional files stay selected and only the non-conventional one is
  silently dropped — rule 3 is the one that notices.
- _A selected file that lacks the convention_ is **rule 2**, and it catches a filter broadened the
...
The confusion was reasonable: the pre-existing fixture for rule 2 put its file _inside_ `src/a11y/`,
...
records as _not ruled on yet_ and [#24](https://github.com/openzigs/onyourleft/issues/24)'s to
...
wording (_"intercepts all network traffic during a map render"_), and it catches what `styleOrigins`
...
minmax(7rem, 1fr))` track whose `7rem` is a _floor_ rather than an `auto`, and a single word has no
...
off, so the same words are laid out at the size they had before #259 — and the spec requires _those_
...
its own it is weak, because with the tokens as they are _deleting_ `min-height` leaves the box at
...
⚠️ **Two controls, and they fail for different reasons.** A green _"it loads offline"_ is
...
not a navigation, because the worker serves the precached shell for _every_ navigation — and (2)
...
has no symbol layers. So the static check catches a third-party URL that is _declared_ and the
browser catches a host that is _contacted_ but declared nowhere. Neither is the other's superset and
...
tool — and this sentence said _four_ until #373, _seven_ until #430's pull request, when it had
been stale for two entries already, _ten_ until #528 and _eleven_ until #529, so read
...
⚠️ **The harness server is a _static file server_, not a single-page app.** `vite.browser.config.ts`
...
`127.0.0.1`, and the one line that would have named the address the server _did_ bind to is exactly
...
D-1 quotes Protomaps' warning that R2 is _"known to have higher latency (500 ms or higher)"_, and
...
bytes**, identical to what the loopback measurement uses to mean _"served from cache"_, and meaning
...
where _weak_ is ADR 0015 D-2's set — `MPL-2.0`, `BlueOak-1.0.0`, `CC0-1.0`, `MIT-0`, `0BSD` — plus
`Unlicense`, added to that set by [ADR 0016](docs/adr/0016-unlicense.md) D-1. `Unlicense` grants
more than MIT does and still sits in _weak_ rather than _permissive_, because the permissive row is
...
⚠️ **And since ADR 0025 they are forbidden in what an _app_ ships, too — which this section used to
say the opposite of.** The table above read _"permissive + weak + GPL/LGPL/AGPL"_ in both `apps/*`
...
over every workspace package is what makes the check complete**, the package list is _discovered_
...
codebase at all. What decides where the _interface_ lives is D-4's other argument: _"the interface
outlives the transport… the moment anyone wants an offline or in-browser route, a permissive engine
can be compiled in and a GPL one cannot."_ An Apache-2.0 leaf is where that door stays open.
...
criteria — _"the same test suite passes against two different engines"_ and _"repointed from a
bootstrap endpoint to a self-hosted instance"_ — cannot be met without one. An adapter written
against an API nobody in the loop can call is §4a's _"a documented command nobody has run is the
most expensive kind of wrong"_ in a new place, and it is **worse than absent**: a reviewer reads
...
coverage; the screen. #71's two hardest criteria are about _how many times_ an engine was asked and
_which legs_ — a real engine answering correctly proves neither, which is why the double counts its
...
**apps/web's build** — that is #85's own premise, _"the mobile shell wraps the same web build, so
there is exactly one client and a divergence between the two is impossible rather than merely
discouraged"_. So a renderer under `apps/mobile/src` would typecheck, test green, and **never be
...
Capacitor partly _for_ web UI reuse. §2's layout already describes `apps/mobile` as the shell. And
...
`apps/mobile` declared no `exports` (so nothing _could_ import it), nothing implemented
...
user agent, because a Capacitor WebView _is_ Chrome and reports itself as Chrome, and not
...
_what may be written_ — `createTrainerControl`, the bounding, the quantisation, the feature gating
...
_"compiles, satisfies every test in this file, and reintroduces exactly the fire-and-forget failure
the client exists to prevent"_ — so `plugin-port.ts` declares the unsafe sibling, `ble-client.ts`
...
`ride/controller.ts` §`releaseTrainer` — _End ERG_, the end of a workout, stopping a ride, and a game
...
most of them the ones §4b records as having no production consumer _by design_. A noisy rule gets an
allowlist, and an allowlist that grows is how a rule stops firing. So the selector is a **principle
rather than a directory**, and the principle is §6's own — _"a smart trainer applies physical
resistance to a person who is pedalling"_. `TRAINER_COMMAND_SEAM` is the five modules that stand
...
that says _"285 production modules"_ and nothing else reads like coverage of a population it never
checked. ⚠️ **The third count is the seam's own — `5 of 5 trainer-command seam modules` — and it is
REPORTED rather than asserted.** `missingSeamFiles` raises when _some_ of the five are absent and
deliberately not when _all_ are, because a tree with none of them is a tree with no trainer in it,
...
exist.** A selector like that fails closed against _deleting_ what it names and open against
_renaming_ it — rename `ride/` to `riding/` and every rule passes over an empty population while the
...
almost every fixture in `check-wiring.test.sh` is — and a tree holding _some_ of them is one where a
...
is how _"an allowlist that grows is how a rule stops firing"_ starts. #438 weighed three options:
...
`unwiredReason` matched it anywhere in the stripped comment, so a paragraph _saying the exemption
had been removed_ parsed as a live one with the rest of the sentence as its reason, and on the
...
reasons.** `check:wiring` asks whether any production declaration _names_
...
anchored tables are. Every share, ratio, threshold and band stated in a _sentence_ is outside every
...
it gets routed around rather than met. Coverage is still _reported_, because an untested branch is
...
### Verifying a _compile-time_ guarantee
...
The same rule binds a _review_: if a fix commit claims a review finding was mistaken, that claim is
...
read cannot see it**, from four causes: _wrong storage_ (it landed in a cache or a different key
prefix), _wrong layer_ (acknowledged at the edge, nothing below persisted), _wrong time_ (written
after the read, or in a transaction that never committed), _wrong harness_ (the test asserted against
...
_cannot_ be served by the handle that wrote: it discards every open handle before it opens another.
...
- **`seedRide` and `streamSetFor` take an owner**, so the write-path scoping case is as short to
  write as the read-path one. #26's review found that a two-athlete _read_ fixture is blind to a
...
- **`fakes.ts` holds thirteen deliberately broken stores**, and the first five are the ones worth
  ...
  _five_ from #61 until #325's `roundedMassStoreFactory` made it thirteen, because every write path
  added since brought one. Count the `StoreFactory` exports rather than reading a number here.
- **A round trip over a _signed_ artefact ends in a verification, not a comparison.**
...
decision gets recorded. Quoting the _SDK source_ is not. See
...
in code, in a UI label, in a metric key or in a column header. The _formulae_ are unaffected — they
...
- **R1** permits taking facts and forbids taking _somebody's compilation_ of them as a table, and an
...
- ⚠️ **An unrecognised key is refused, not ignored** (D-4) — the opposite of the usual convention.
  A future field that changed what a workout _does_ would otherwise be dropped silently and the
...
- ⚠️ **The format is in `packages/domain`, not `packages/fit`** (D-5). `packages/fit` is where
  _other people's_ formats live and its identity is ADR 0006's clean-room posture; a serialisation
...
corpus of zero. #14's fifth criterion — _"at least 50 workouts from the existing open ZWO corpus
parse"_ — **is not met and is not claimed to be**; ADR 0017 supersedes it. Importing that format is
...
- **Cross-athlete exposure.** Any query matching on an entity id **without also filtering on the
  ...
  `packages/store/src/activity-store.scoping.test.ts` exists and why its enumeration is _derived_
...
- **A closing keyword must be correct when the pull request is OPENED.** GitHub creates the
  ...
  work. Prose that merely _mentions_ a keyword counts too: #29 was closed by `Resolves #29`
...
- **ADRs**: `docs/adr/NNNN-kebab-case.md`, with **Status, Context, Decision, Consequences** —
  ...
  **Q6 lifted D-0's counsel block** while its "#7, no server in Phase 1" block stands, so nothing
  ...
  ⚠️ This sentence said _"every number from 0001 to 0020 is now written and the next free number is
  0021 — there is no live reservation"_, and then that **0021 was a live reservation** claimed by
  ...
  _check_ this bullet asks for is `docs/architecture.md`'s reservation table, not this sentence; a
  ...
  **0018 is [ADR 0018](docs/adr/0018-native-client-platform.md)**, taken by
  ...
  ADR 0001's _Data_ deferral had no number for
...
to read _"Vitest 5 is in release candidate"_; `npm view vitest dist-tags` reported `latest:
...
_fork's_ `pnpm-workspace.yaml`, so flipping an entry to `true` and adding a dependency is what makes
...
WebBluetoothCG's own status file says _"Linux is partially implemented and not supported"_. #40's
...
| What a _shared_ copy of a ride contains, and why the rider's own view is not trimmed                                                                                                                                 | [ADR 0004](docs/adr/0004-privacy-and-location.md) decisions B, C and E, `apps/web/src/detail/privacy.ts`, `apps/web/src/transfer/export-activity.ts`                                                                                                                                       |
...
| Why the hillside beside the road is invented rather than the real one, and what a real one would cost                                                                                                                | [ADR 0032](docs/adr/0032-external-data-for-the-game-world.md) D-2, `apps/web/src/game/landform.ts` §"Where every height comes from", [#458](https://github.com/openzigs/onyourleft/issues/458)                                                                                             |
...
| Why a bend is drawn as a curve rather than as the route's own straight pieces, what it cuts off a corner, and what it does not move                                                                                  | `apps/web/src/game/terrain.ts` §`BEND_SMOOTHING_METRES`, §`CORRIDOR_STEP_METRES`, §`drawnGroundPosition`, `apps/web/browser/bend-harness.ts`, [#543](https://github.com/openzigs/onyourleft/issues/543)                                                                                    |
...
| Why the wake lock's _release_ is the half that is tested                                                                                                                                                             | `apps/web/src/game/hud/wake-lock.ts`                                                                                                                                                                                                                                                       |
...
| Who asks the trainer for control in the game, why it is the rider's Ride press and not the screen, and why the port method is not called `requestControl`                                                            | `apps/web/src/game/trainer-port.ts` §"Who asks for control, and when", §`GameTrainerPort.askForControlOnRide`, [#503](https://github.com/openzigs/onyourleft/issues/503)                                                                                                                   |
| Why control is its own step on the Ride screen, and ERG only one thing to do with it                                                                                                                                 | `apps/web/src/ride/TrainerPanel.tsx` §"Control is its own step", [#503](https://github.com/openzigs/onyourleft/issues/503)                                                                                                                                                                 |
