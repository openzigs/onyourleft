...
apps/                 AGPL-3.0-or-later, without exception
  web/                browser client — the Phase 1 product (#48-#51)
    ...
    src/game/terrain.ts the road as geometry (#91), and since #242 as a road: two
                        ...
                        the route over 10 m either side, and the corridor steps
                        every ~2 m rather than every grid point, because a
                        planner's GPX turns 25° to 45° a sample and drew as
                        straights meeting at corners. A point's route distance,
                        height and gradient are still the centreline's; how far
                        the corridor reaches is still the grid's, which is why
                        the arrangement digest did not move; and the ground
                        beside it has ~5× the rows. `unsmoothed` is the control
