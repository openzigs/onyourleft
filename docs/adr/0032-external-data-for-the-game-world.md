...
- **2026-09-26** — **Two statements of fact in D-1's ESA WorldCover ruling are now false, and D-1 itself is unchanged by this note.** Item 3 says [#53](https://github.com/openzigs/onyourleft/issues/53) *"has published nothing — `.env.example`'s `VITE_BASEMAP_PMTILES_URL` is still empty"*, and D-2's table says the same in its first row. Item 2 and D-2's fifth row call a runtime fetch *"the first outbound HTTP request `apps/web/src` has ever made"*, and §Consequences §"What this enables" says *"`apps/web/src` keeps its zero outbound requests"* — false now for the same two reasons, (2) and the `fetch` below. What changed, [#534](https://github.com/openzigs/onyourleft/issues/534) and [#535](https://github.com/openzigs/onyourleft/issues/535): **(1) the basemap archive is published, and every build uses it by default.** #53 published `https://tiles.openzigs.com/basemap-us-20260914.pmtiles` on 2026-09-14, and since #534 a build with nothing configured draws it — `apps/web/src/map/basemap.ts` §`PUBLISHED_BASEMAP_URL`. `VITE_BASEMAP_PMTILES_URL` is still empty in `.env.example`, but empty now **means** that archive rather than no map, and the template documents it as the default. **(2) The app therefore contacts `tiles.openzigs.com` by default whenever a ride's map is shown** — the ride detail view, for a ride with GPS — and that request carries the rider's IP address. It is issued by MapLibre rather than by code of ours in `apps/web/src`, which is why item 2's wording can be read as still literally true; a reader should not read it that way, because the tile host **is** contacted. Since the owner's decision of 2026-09-25 on #535, **a rider can turn map tiles off** in Settings (`apps/web/src/map/tiles-preference.ts`), and off builds a style with no source at all, so nothing is asked of the tile host. Tiles are **on** by default. ⚠️ Separately, and also since this ADR was written, `apps/web/src/camera/analysis-transport.ts` ([#387](https://github.com/openzigs/onyourleft/issues/387)) makes one `fetch` of its own — to the rider's own computer, never to a host of ours — so *"the first outbound request in `apps/web/src`"* is no longer available to any future world fetch either; `apps/web/src/privacy/no-network.test.ts` §`PERMITTED_NETWORK_CALLS` is where every permitted call is now enumerated. **(3) ADR 0032's decision is unchanged.** The trainer game's world still takes **no** external data source. The basemap is the **ride-detail map**, not the game world — this ADR's own constraints table already said *"The basemap is a map, not a world; D-1 is about `apps/web/src/game/`"* — and item 2's substantive argument stands in full: a world request would be made **continuously while somebody rides**, saying where they are *riding* rather than where they are *looking*, and a switch on a map shown after the ride answers none of that. Only the factual premises of items 2 and 3 have moved. Item 3's hosting objection is **weaker** than written — somebody does now serve a PMTiles archive — but it was one of three counts, and the global 10 m raster D-1 rejects is still not that archive and still has nowhere to live. This is a correction of fact under [ADR 0013](0013-adr-amendments.md) D-3; nothing a reader should **do** changes, so it is an amendment rather than a superseding ADR.
