...
⚠️ **A spike number is therefore NOT unique by any rule, and 0006 was taken twice on 2026-09-22** —
[#493](https://github.com/openzigs/onyourleft/issues/493) is the rule that would have caught it.
Until it exists, the guard is to read `docs/spikes/` **and every open pull request's file list** at
write time, and to add the row below **in the same commit**: a row is what turns the next collision
into a merge conflict somebody sees, where two differently-named new files are a clean merge.
...
| 0032 | #248 — whether the game world ever takes an external data source | [Written](adr/0032-external-data-for-the-game-world.md), 2026-09-22. ⚠️ **Took 0032 rather than 0029, and the gap is deliberate**: 0029, 0030 and 0031 were claimed the same day by the camera-decision work ([#378](https://github.com/openzigs/onyourleft/issues/378)–[#381](https://github.com/openzigs/onyourleft/issues/381)) in a parallel pull request, and a written ADR cannot be renumbered without breaking citations. Also discharges [#376](https://github.com/openzigs/onyourleft/issues/376), which asked for the street-level imagery ruling to land **in this document rather than in a second ADR**. **The decision is no external data source for the trainer game's world** — every candidate in #248's table ruled on, each refused on a rule the owner has already taken ([ADR 0002](adr/0002-local-first-architecture.md) decision A, [ADR 0010](adr/0010-map-tiles-and-routing.md) D-7) or on a licence that fails closed. Two candidates are **upgraded from second-hand to first-hand and still refused**: **ESA WorldCover** is genuinely `CC-BY-4.0` with the reported attribution string, which `ASSET004`, `ASSET006` and the #358 credits screen could discharge — it fails on there being **nowhere to put a global 10 m raster**, not on its licence; and **SRTM/NASADEM/3DEP** are genuinely public domain and CC0, and fail on 30 m being the wrong scale for a hillside at road width. **Copernicus GLO-30's post-2026 status is recorded as UNRESOLVED**, with what was read and what refused to load, so anything resting on it inherits an open dependency — [ADR 0010](adr/0010-map-tiles-and-routing.md) open question 4, still #72's. ⚠️ **ADR 0010 open question 6 is ANSWERED** (D-6): the third attempt at Mapterhorn's attribution page succeeded on 2026-09-22 after a 404 in 2026-09-03 and a 403 in 2026-09-11, and the answer is that **there is no single licence** — the data is the union of **153** national and regional open-data licences curated under a policy excluding share-alike and non-commercial, and the project's own public attribution surface is one `© Mapterhorn` line whose expansion **404s**. ⚠️ **D-3 is the boundary a later reader most needs**: the **route's** elevation is [ADR 0010](adr/0010-map-tiles-and-routing.md) D-5's and #72's and is **not** touched here; an issue that "just reads the DEM the route already used, for the ground beside it" has crossed into this ADR's subject and must supersede it. |

**The next free number is 0033.** Every number from 0001 to 0028 is written — **0021 included, since
2026-09-22** — 0032 was taken by #248, and **0029, 0030 and 0031 are live reservations** claimed by
the camera-decision work ([#378](https://github.com/openzigs/onyourleft/issues/378)–[#381](https://github.com/openzigs/onyourleft/issues/381))
in a pull request opened the same day as #248's and #330's. ⚠️ **This sentence said *"the next free
...
is to leave a gap rather than to race. #248 left one on purpose. ⚠️ **If the 0029–0031 reservations
are abandoned, those numbers become free and this list becomes non-contiguous permanently** — which
is what 0012 and 0021 have each already cost once, and is the reason this table exists rather than
the sentence.
