...
| [0010](spikes/0010-on-device-pose-model-cost.md) | What does a pose model cost on the Pixel Tablet at 5 pictures a second **while the trainer game renders**, and whose weights may this repository ship? ([#385](https://github.com/openzigs/onyourleft/issues/385)'s on-device half, for [#530](https://github.com/openzigs/onyourleft/issues/530)) | In a Web Worker on the CPU, every model measured cost the ride **no frames**. On the main thread every one missed 191–203 vsyncs in 40 s. The cheapest beside the ride was MediaPipe Pose Landmarker lite on its CPU delegate, at 57.9 / 73.6 ms p50 / p95 per picture. MoveNet's weights are **Apache 2.0 at Google's own Kaggle page and model card**, which settles what #328 left open. RTMPose states no weights licence and fails closed. The same inference costs 2.3–2.7 times as much at 5 per second as back to back | Chrome 153 on the tablet, 2026-09-25, with the product's renderer from `realistic.html` as the load. ⚠️ **The accuracy half of #385 was not done**, because nobody could ride. It is not the app's WebView, and its frame figures are missed-vsync counts, not #323's `gfxinfo` 24 ms. **0010 rather than 0009** because [#471](https://github.com/openzigs/onyourleft/pull/471), still open, claims 0009 |
