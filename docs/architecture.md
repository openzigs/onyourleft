...
| The side camera's phone (#528, [ADR 0033](adr/0033-side-camera-link.md) D-3, D-5, D-7, D-8) | `camera/side-camera.ts` — the session: framing, filming on the tablet's *start*, and **stopped at 30 s on the phone's own timer** when the link is lost, with a countdown; `side-camera-link-port.ts` — what the phone needs from #529's link (⚠️ this cell said the link did not exist yet; since #529 it does, in the row below, and the port gained `connecting` and `ended`); `framing.ts` — the reference as numbers, the check and its three tolerances with their provenance, the guide outline; `views/SideCameraView.tsx` at `#/camera/side` — the consent with D-5's bystander sentence **and** ADR 0033's 30-second sentence on this device, the live preview with the guide and last time's ghost, the full-screen sign and one stop control | `side-camera.test.ts` asserts the camera's own `stopCamera` at 30 000 ms and not at 29 999, over a real `CameraController`; `framing.test.ts` holds every tolerance from both sides; `SideCameraView.test.tsx` and `.a11y.test.tsx` for the four states; `browser/sidecamera.browser.spec.ts` for the sign's dominance and the one 44 × 44 control at five phone viewports, with a control |
| The side-camera link (#529, [ADR 0033](adr/0033-side-camera-link.md) D-1, D-3, D-4, D-5, D-9, D-10) | `camera/side-link-transport.ts` — **the one naming of the platform's peer connection**, pointed at `{ iceServers: [] }` and nothing else; `side-link-sdp.ts` — the few fields of a description read out, and the other end's description **rebuilt** from checked fields so nothing a peer wrote reaches `setRemoteDescription`; `side-link-code.ts` — the two QR codes' format (`OYLSIDE1{…}`), an unknown field refused, D-4's candidate rule as `addressSpaceOf` behind a bracketing adapter, TCP candidates left out by the encoder, names-only codes NOT refused; `side-link-messages.ts` — D-3's two columns and nothing else; `side-link.ts` — both ends: the one-time secret as the phone's first message, a 1 s heartbeat and **3 s of silence is link lost**, acknowledged `start`/`stop` reported `unacknowledged` after 3 s, a graceful channel close before the connection; `side-pairing-port.ts` — where a pairing comes from, and the tablet's end, held by the port so it outlives the Camera screen — once answered; an **unanswered** offer is voided when the Camera screen closes (D-4), and expires after five minutes if the screen is left open; `side-link-qr.ts` — `uqr` to draw, `jsqr` to read, **loaded with `import()` only when a code is shown or read** (`PairingCode.tsx`, `session.ts` §`readPairingCode`), so neither library is in the entry chunk (#550's review); `views/SideCameraControl.tsx` on the Camera screen, and pairing inside `views/SideCameraView.tsx`. ⚠️ **Since #557** the tablet reads the phone's code with its FRONT camera and shows `camera/ScanViewfinder.tsx` while, and only while, it reads (ADR 0033's 2026-09-26 amendment); the pairing holds a screen wake lock from the offer to its end (`side-link.ts` §`SidePairingOptions.screenLock`), taken again each time the page comes back into view since #566's review, because the platform lets it go whenever the page is hidden (`game/hud/wake-lock.ts` §`browserScreenLockSource`, which the ride's lock shares); and a pairing the phone closed after *Stop filming* on the tablet is reported as ended on the tablet | `side-link.test.ts` pairs both ends through the real codes over `testing.ts` §`sidePeerNetwork` and **drives the drop at both ends** — the tablet says *link lost* at 3 s, the phone's camera stops at 30 s; `side-link-code.test.ts`, `side-link-sdp.test.ts`, `side-link-messages.test.ts`, `side-link-transport.test.ts` (D-9 step 3); `no-network.test.ts` (D-9 steps 1 and 2); `browser/sidelink.browser.spec.ts` pairs two real peer connections in the pinned Chromium. ⚠️ **Not run on two devices by this change** — spike 0012 measured the configuration, not this code |
| The side camera during a ride (#551, [ADR 0033](adr/0033-side-camera-link.md) D-5, the owner's ruling on #527) | `ride/side-camera.ts` — pure: the ONE line a ride shows (filming, stopped, **link lost**, and on-but-not-filming, stopping, a stop the phone did not confirm, a session that ended some other way), whether *Stop side camera* is offered (while the phone films or the link is lost, and not once the pairing has ended), and the one sentence said when the link goes — **when it goes, never on a first render and never again every frame**; `ride/useSideCamera.ts` — reads `side-pairing-port.ts` §`currentSideCamera` on every render rather than holding it; `ride/SideCameraOnRide.tsx` on the Ride screen; `game/hud/HudPanel.tsx` §`sideCamera` in the HUD's actions panel as one row with a 44 px stop, and **a lost link in the notice slot** (`GameView` §`notices`) — the steady line is NOT in the notice slot, because on a phone a notice takes the route panel's cell (#437). The sentence goes through the screen's ONE region (`game/hud/announce.ts` §`side-camera-lost`, ranked below the climb and above every reading, and **spoken with announcements off**, as the Camera screen's `role="status"` was); no second live region, and **no picture**: nothing on the ride's path reads the pictures' channel or the analysis | `ride/side-camera.test.ts` (including a source scan for the pictures' names); `game/side-camera-hud.a11y.test.tsx` and `ride/side-camera-ride-screen.a11y.test.tsx` through the real `AppShell`, counting the lost link at the announcer's entrance; `game/hud/announce.test.ts` §"#551"; `browser/ride.browser.spec.ts` §"a ride with a side camera paired" at every overlay viewport, with a control. ⚠️ **Sounds on AND a side camera together do NOT fit a phone's actions panel** — each alone fits, together the panel is 228 px on a Mac and runs off the stage at 736×360 and onto the rider at 360×800 and 360×752; tablets fit. [#576](https://github.com/openzigs/onyourleft/issues/576). The stop is **44 px, the mute's size, not the HUD's 72**, because a 72 px stop did not fit a 736×360 phone on the CI runner's fonts even alone (`HudPanel.tsx` §`sideCamera`) |
| The side camera's pictures, and the pose model on the tablet (#530, [ADR 0033](adr/0033-side-camera-link.md) D-3, D-5, D-6, D-7, D-10) | `camera/side-link-pictures.ts` — one picture on the `frames` channel (unordered, no retransmission): nine bytes of header — a version, a sequence number, milliseconds since filming began — and a JPEG, and the refusal of anything else, a picture carrying a metadata marker included; `side-camera.ts` — the phone takes a 256-pixel picture (`browser-camera.ts` §`videoSidePictureSampler`, through `frame.ts` §`capturedFrame`) every 200 ms while filming **and connected**, one at a time, and drops one taken as the link went; `side-link.ts` — both ends of `frames`, the connection's own `maxMessageSize` read before a send, one picture in flight; `side-analysis.ts` — on the tablet, **each picture looked at as it arrives and dropped**, at most one waiting (the newest), **pose numbers kept keyed by sequence and milliseconds only**, the framing check against the stored reference and the verdict sent to the phone, this session's placement stored as the next reference **only when its check was `matches` or there was none yet (`no-reference`)** — #388's owner ruling, `REFERENCE_MOVES_ON`, so a tripod that creeps each session is still caught; `pose-estimator.ts`, `pose-worker.ts`, `pose-landmarks.ts`, `pose-runtime.ts` — MediaPipe Pose Landmarker lite on the CPU in a module worker (spike 0010 §7's candidate), the near side's nine landmarks at visibility ≥ 0.5, the weights committed in `public/pose/` (`ASSETS.toml`, ADR 0031 D-1) and the runtime copied from the pinned package by `tools/pose/pose-runtime-plugin.ts`, **`pose/` excluded from the precache**; `pose-fence.ts` — ⚠️ **the worker refuses every request off its own origin, because `@mediapipe/tasks-vision` 1.0.1 posts usage logs to `odml.pa.googleapis.com`** unless stopped; `views/SideCameraControl.tsx` — counts and the framing check, never a picture | `side-link-pictures.test.ts`, `side-camera.test.ts` §"the pictures", `side-link.test.ts` §"pictures", `side-analysis.test.ts`, `pose-*.test.ts`, `precache.test.ts` §"the side camera's pose model", `no-network.test.ts` §"the pose worker's network fence"; `browser/pose.browser.spec.ts` runs the real worker on a CC0 photograph of a rider (a pose), a blank picture (nobody) and noise (unreadable), and watches every request — **measured red with the fence removed**, naming the Google host; `browser/sidelink.browser.spec.ts` sends three real pictures across. ⚠️ **Not built: the rider's own computer as the analyser on this path** (ADR 0033 D-11, [#553](https://github.com/openzigs/onyourleft/issues/553)), **Not run on the tablet in this build** — spike 0010 measured the model and delegate, not the module runtime inside the APK ([#554](https://github.com/openzigs/onyourleft/issues/554)) |
| The side camera's post-ride report (#388, [ADR 0030](adr/0030-what-the-app-may-say-about-a-body.md) D-1–D-8, ADR 0033 D-6) | `camera/side-report.ts` — pure: the session's pose numbers in, **sentences** out, comparing the first third of the session with its last third on the same camera, **sagittal only** (torso, knee at the bottom of the stroke, elbow, head fore and aft, fore and aft on the saddle), each threshold a named provisional constant; `ObservedChange` with degrees is computed by `observedChanges` and **rendered by nothing** while `MEASURED_SPREAD_DEGREES` is absent (#385); `camera/side-report-wording.ts` — **every sentence**, each worded "possibly", no number, the one vocabulary file ADR 0030 D-8 asks for; `camera/side-report-port.ts` + `side-report-keeper.ts` — which ride the session filmed (the one saved while it was open, or after it ended while that ride was under way; a ride must have been seen under way) and the save; `detail/SideCameraSection.tsx` — the **Side camera** section on the ride's own page, only for a ride with a report, rendering only sentences the vocabulary file can produce, with an explicit *nothing to show* and no control; `packages/store` §`SideCameraReportRecord`, schema version 12 — sentences only, keyed by the activity, read by athlete and activity, in `deleteActivity`'s and `deleteAthlete`'s cascades, `ERASE_REMOVES` and each ride's manifest entry | `side-report.test.ts` (one posture change at a time, mirrored), `side-report-keeper.test.ts` (against the real store), `side-camera-report-store.test.ts` with `lastSentenceDroppedReportStoreFactory`, the derived scoping probe and erasure table list, `SideCameraSection.test.tsx` and `.a11y.test.tsx`, `export-everything.test.ts`, `erase-device.test.ts`; **`camera/no-absolute-angles.test.ts` — the gate: no string in `apps/web/src` renders a degree sign, the word, a `'degree'` formatter or a frontal-plane word, three non-body exemptions each pinned to its text**; `camera/side-report-safety.test.ts` — the report path's import closure reaches no trainer module. ⚠️ **No number and no cross-session sentence**: numbers wait for #385's measured spread, and a comparison with an earlier session needs retained pose numbers, which the owner put out of scope. A green scan is not evidence ADR 0030 was followed (D-8) |
| The framing reference (#528, ADR 0033 D-7) | `packages/store` §`FramingReferenceRecord`, schema version 11 — keyed by the athlete, numbers only; in `deleteAthlete`'s cascade, `ERASE_REMOVES` and the account manifest. its production writer is #530's `camera/side-analysis.ts`, which also records `check` — whether that session's framing check passed (`matches`, `differs`, `no-reference`, `not-checked`; absent means not recorded and fails closed), so #388 compares two sessions only on `matches` | `framing-reference-store.test.ts` with `firstReferenceStoreFactory`, the derived scoping probes and erasure table list, `export-everything.test.ts`, `erase-device.test.ts` |
...
| WebRTC | **out for this path.** ⚠️ The reason given here until #527 was that WebRTC *"needs signalling infrastructure, which is a server (owner decision D6)"*. That is not true of two devices in one room: [ADR 0033](adr/0033-side-camera-link.md) supersedes ADR 0029 D-6's WebRTC row for the **side camera**, where the offer and answer cross by QR code and no ICE server is configured. The rider's computer is not a browser peer, so this path is unchanged |
...
| [0010](spikes/0010-on-device-pose-model-cost.md) | What does a pose model cost on the Pixel Tablet at 5 pictures a second **while the trainer game renders**, and whose weights may this repository ship? ([#385](https://github.com/openzigs/onyourleft/issues/385)'s on-device half, for [#530](https://github.com/openzigs/onyourleft/issues/530)) | In a Web Worker on the CPU, every model measured cost the ride **no frames**. On the main thread every one missed 191–203 vsyncs in 40 s. The cheapest beside the ride was MediaPipe Pose Landmarker lite on its CPU delegate, at 65.8 / 80.9 ms p50 / p95 per picture over a ten-minute soak (57.9 / 73.6 in its best 40 s run). MoveNet's weights are **Apache 2.0 at Google's own Kaggle page and model card**, which settles what #328 left open. RTMPose states no weights licence and fails closed. The same inference costs 2.3–2.7 times as much at 5 per second as back to back | Chrome 153 on the tablet, 2026-09-25, with the product's renderer from `realistic.html` as the load. ⚠️ **The accuracy half of #385 was not done**, because nobody could ride. Mostly Chrome: the two leading models were also run in the app's own WebView (§5.5), where `gfxinfo` frame p50 went from 10–12 ms to 13 ms on a scene lighter than #323's 24 ms, one run per row. The rest of its frame figures are missed-vsync counts, not `gfxinfo`. **0010 rather than 0009** because [#471](https://github.com/openzigs/onyourleft/pull/471), still open, claims 0009 |
| [0011](spikes/0011-webrtc-host-only-on-the-lan.md) | Can a WebRTC data channel open on the LAN with host candidates only, no STUN or TURN, as [ADR 0033](adr/0033-side-camera-link.md) D-1 needs? ([#532](https://github.com/openzigs/onyourleft/issues/532), **partial**) | For the three pairs measured, **works with a named condition**: at least one end must publish a raw private address. Chrome ↔ Chrome on one Mac, tablet Chrome ↔ Mac Chrome and the app's WebView ↔ Mac Chrome all opened in under 100 ms and carried 5 frames/s of 15–25 KB for 10 minutes with at most 1 frame lost in 3000. ⚠️ **The Android end, in Chrome 153 and WebView 153, cannot resolve a Chrome mDNS `.local` candidate** (`ERR_NAME_NOT_RESOLVED` in logcat). Pairs with Android in them connected only because Android published its raw IPv4 address and the Mac dialled it. Desktop Chrome hid its address behind mDNS until the page had camera access, which confirms D-1's expectation | One Pixel Tablet (Android 17), macOS Chrome 154, one TP-Link home router, 2026-09-25. ⚠️ **The pair #532 names, WebView ↔ WebView on two Android devices, is UNMEASURED**: there was one Android device. So its pull request says `Refs #532`, and no amendment is appended to ADR 0033. There was **no packet capture** (no root). The "nothing left the LAN" evidence is `iceServers: []`, every candidate pair from `getStats()`, Chrome's own WebRTC log and `nettop`, and §5 names what is still owed |
| [0012](spikes/0012-webrtc-webview-to-webview.md) | Can the app's WebView on one Android device open a host-only WebRTC data channel with the app's WebView on another, the pair [spike 0011](spikes/0011-webrtc-host-only-on-the-lan.md) could not measure? ([#532](https://github.com/openzigs/onyourleft/issues/532)) | **Yes, both ways**: points to **works with a named condition**, the condition being that the router must not isolate Wi-Fi clients (not tested). Both WebViews published their **raw private IPv4 address**, never a `.local` name, TCP or IPv6 candidate, so nothing had to be resolved and 0011's mDNS failure never arises. Either device could dial the other alone; withholding both ends' candidates never opened. Channels opened 72–158 ms after the answer was applied, over all 15 opened runs. Two 10-minute streams, 5 frames/s of 15–25 KB phone → tablet: 6000 of 6000 frames, none reordered, echo RTT p50 64–72 ms. Camera access on the tablet changed nothing, in either order, though its page already had an earlier camera grant, so that is not a clean before/after; the phone published its raw address with no camera access at all | Pixel Tablet and Pixel 8, both Android 17 and WebView 153.0.8010.36, debug builds of the app, one TP-Link router on 5 GHz, 2026-09-25. ⚠️ **No packet capture** (no root), so #532's third criterion is still open (its remainder is tracked in [#541](https://github.com/openzigs/onyourleft/issues/541)), its pull request says `Refs #532`, and no amendment is appended to ADR 0033. The egress evidence is `iceServers: []`, both ends' `getStats()`, both devices' socket tables for the app's uid, and `logcat`. The phone's camera was not opened (the app does not hold `CAMERA`), and client isolation, a release build and other Android versions are untested |
...
**The next free number is 0034.** 0033 was taken by [#527](https://github.com/openzigs/onyourleft/issues/527) on 2026-09-25. Every number from 0001 to 0028 is written — **0021 included, since
