...
There is no account, and the app has no analytics. **Your rides stay on your device.** To draw a map, the app
asks our tile server for the map around where you rode. It sends no ride data. **Cloudflare, which
runs the tile server for us, keeps a record of recent requests — including your IP address and which
map tiles were asked for, which is roughly where you rode — for up to 7 days**, as part of the
standard traffic analytics it keeps for every site it serves. We do not use that record or share
...
identifier of ours, but the host sees your IP address and roughly which area you are looking at, and
Cloudflare keeps that for up to 7 days. **You can turn it off**: Settings → *Ride map* → *Draw map tiles under my rides*. With it off the app
...
leaves the device**: which tiles were asked for says roughly where you rode, and Cloudflare keeps
that, with your IP address, for up to 7 days. You can turn it off.
...
- **Map tile requests, when a map is on screen.** Map tiles are **on by default**, and you can turn
  ...
  trimmed. **Cloudflare, which runs the host for us, keeps a record of recent requests — including
  your IP address and the path of each tile asked for — for up to 7 days**, as part of the standard
  traffic analytics Cloudflare keeps for every site it serves, which cannot be switched off. Seven
  days is the limit for this project's Cloudflare plan (Cloudflare's
...
- No sale or sharing of personal information. Nothing is transmitted to us or to anybody else
  ...
  above, whose record Cloudflare keeps for us for up to 7 days and which we neither use nor share.
